...
public class Leaderboard : MonoBehaviour {
    ...
    void Start() {
        ...
        current_player_time = Mathf.Abs(PlayerPrefs.GetFloat("current_time", 0.0f));
        ...
        // Load Scores
        score1 = Mathf.Abs(PlayerPrefs.GetFloat("score1", 0.6f));
        score2 = Mathf.Abs(PlayerPrefs.GetFloat("score2", 0.1f));
        score3 = Mathf.Abs(PlayerPrefs.GetFloat("score3", 0.2f));
        score4 = Mathf.Abs(PlayerPrefs.GetFloat("score4", 0.3f));
        score5 = Mathf.Abs(PlayerPrefs.GetFloat("score5", 0.4f));
        // Load Kills
        ...
        foreach (KeyValuePair<float, ArrayList> n in sl) {
            if (rank <= 5) {
                //Debug.Log("Time: " + Mathf.Abs(n.Key).ToString("F1") + ", Name: " + n.Value[0] + ", Kills: " + n.Value[1]);
                Section _p = Instantiate(row_prefab, board.transform);
                _p.SetText(rank, n.Value[0].ToString(), Mathf.Abs(n.Key), (int)n.Value[1]);

                // SAVE
                PlayerPrefs.SetString("name" + rank.ToString(), n.Value[0].ToString());
                PlayerPrefs.SetFloat("score" + rank.ToString(), n.Key);
                PlayerPrefs.SetInt("kills" + rank.ToString(), (int)n.Value[1]);
